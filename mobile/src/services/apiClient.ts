...
export class ApiClient {
  ...
  /** 视频上传方法 - 支持视频和封面上传，带进度回调 */
  async uploadVideo<T = any>(
    endpoint: string,
    {
      title,
      content,
    ...
    // 视频封面现在是必需的，总是添加到表单数据中
    formData.append('videoCover', videoCover);

    // TODO: 在实际实现中可以使用 XMLHttpRequest 来获取上传进度
    // 目前 fetch API 不支持原生的上传进度跟踪
    if (onProgress) {
      // 模拟进度更新，实际应该使用 XMLHttpRequest
      onProgress(0);
