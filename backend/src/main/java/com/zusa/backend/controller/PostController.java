...
public class PostController {
    ...
    public Page<PostSummaryDto> feed(
            ...
            @RequestParam(required = false) MediaType mediaType, // [VIDEO-CREATE] 支持媒体类型筛选
            Pageable pageable,
        ...
        // [VIDEO-CREATE] 调用支持媒体类型筛选的Feed方法
        return postService.listFeed(type, mediaType, me, pageable);
    }
    ...
    public static class CreatePostForm {
        ...
        // [VIDEO-CREATE] 开始 - 支持视频字段
        MediaType mediaType; // 媒体类型：IMAGE 或 VIDEO，默认为 IMAGE

        @Size(max = 9)
        List<MultipartFile> images; // 图片文件（仅当mediaType=IMAGE时使用）

        MultipartFile video; // 视频文件（仅当mediaType=VIDEO时使用）
        
        MultipartFile videoCover; // 视频封面（仅当mediaType=VIDEO时使用）
        // [VIDEO-CREATE] 结束

        Set<String> tagNames;

        public CreatePostCmd toCmd() {
            return new CreatePostCmd(
                title, 
                content, 
                mediaType != null ? mediaType : MediaType.IMAGE, // 默认为图片
                images, 
                video, 
                videoCover, 
                tagNames
