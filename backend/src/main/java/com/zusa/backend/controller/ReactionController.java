...
public class ReactionController {
    ...
    public ResponseEntity<PostDetailDto> toggle(@PathVariable UUID postUuid,
                                                @RequestBody @Validated ReactionReq req) {

        Reaction.Type type = Reaction.Type.valueOf(req.type.toUpperCase());

        // 1) 执行点赞/收藏切换
        reactionService.toggleReaction(postUuid, req.userUuid, type);

        // 2) 强制刷新，使事务内数据对后续查询可见
        reactionService.flush();

        // 3) 返回最新帖子详情，带 likedByMe / collectedByMe
        PostDetailDto dto = postService.getDetail(postUuid, req.userUuid);
    ...
    public ResponseEntity<Boolean> check(@PathVariable UUID postUuid,
    ...
    public static class ReactionReq {
        ...
        @NotNull UUID userUuid;      // 用户 UUID，由前端显式传入
    }
}
